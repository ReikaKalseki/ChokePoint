...
function runTile(dx, dy)
	local sz = 48--64--32
	local rx = dx/sz*Config.terrainScale
	local ry = dy/sz*Config.terrainScale
	
	local f = 0.1
	local f2 = 0.25
	
	local ox = (SimplexNoise.Noise2DCompound(rx*f, ry*f, {1.25, 1.5, 2, 3, 4, 6, 7, 10, 15, 30}, {0.8, 0.95, 0.9, 0.75, 0.66, 0.55, 0.4, 0.25, 0.15, 0.08}))
	local oy = (SimplexNoise.Noise2DCompound(rx*f+53487, ry*f-102478, {1.25, 1.5, 2, 3, 4, 6, 7, 10, 15, 30}, {0.8, 0.95, 0.9, 0.75, 0.66, 0.55, 0.4, 0.25, 0.15, 0.08}))
	
	local hx = 0.95+math.sin(rx+ox)
	local hy = 0.95+math.sin(ry+oy)
	
	local bumpx = math.sin(rx*4.7-297854)
	local bumpy = math.sin(ry*4.3+98345)
	--game.print(dx .. " > " .. bumpx)
