...
function runTile(dx, dy)
	...
	local val = (SimplexNoise.Noise2DCompound(rx*Config.terrainScale*f, ry*Config.terrainScale*f, {1.25, 1.5, 2, 3, 4, 6, 7, 10, 15, 25, 35, 60}, {0.8, 0.95, 0.9, 0.75, 0.66, 0.55, 0.4, 0.25, 0.15, 0.1, 0.08, 0.06})) --range from -1 to +1
	...
	local val4 = (SimplexNoise.Noise2DCompound(rx*3*Config.terrainScale+289574*d, ry*3*Config.terrainScale+847365*d, {1.25, 1.5, 2, 3, 4, 6, 7, 10, 15, 25, 35, 60}, {0.8, 0.95, 0.9, 0.75, 0.66, 0.55, 0.4, 0.25, 0.15, 0.1, 0.08, 0.06})) --range from -1 to +1
	
	local any = val3 < 0.85
	local large = math.abs(val) < 0.2*(math.sqrt(f))
	local small = math.abs(val4) < 0.25*(math.sqrt(f*d))
	
	if (large or small) and any then
		return (math.abs(val) < 0.075 and (val2 >= 0 or math.abs(val) < 0.02)) and 2 or 1
	end
	return 0
end
