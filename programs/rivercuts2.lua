...
function runTile(dx, dy)
	...
	local val4 = (SimplexNoise.Noise2DCompound(rx*5*Config.terrainScale+78403, ry*5*Config.terrainScale+34598, {1.25, 1.5, 2, 3, 4, 6, 7, 10, 15, 25, 35, 60}, {0.8, 0.95, 0.9, 0.75, 0.66, 0.55, 0.4, 0.25, 0.15, 0.1, 0.08, 0.06})) --range from -1 to +1
	local val5 = (SimplexNoise.Noise2DCompound(rx*1.5*Config.terrainScale-74404, ry*1.5*Config.terrainScale+13345, {1.25, 1.5, 2, 3, 4, 6, 7, 10, 15, 25, 35, 60}, {0.8, 0.95, 0.9, 0.75, 0.66, 0.55, 0.4, 0.25, 0.15, 0.1, 0.08, 0.06})) --range from -1 to +1
	if (math.abs(val) < 0.2 or (math.abs(val4) < 0.35 and val5 > 0.8)) and math.abs(val2) > 0.25 then
		return (math.abs(val) < 0.1 and math.abs(val2) > 0.4 and val3 <= 0) and 2 or 1
	end
	return 0
end
