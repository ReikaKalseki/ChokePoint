...
function runTile(dx, dy)
	local rx = dx/600
	local ry = dy/600
	local valA = (SimplexNoise.Noise2DCompound(rx*Config.terrainScale, ry*Config.terrainScale, {1.25, 1.5, 2, 3, 4, 6, 7, 10, 15, 25, 35, 60}, {0.8, 0.95, 0.9, 0.75, 0.66, 0.55, 0.4, 0.25, 0.15, 0.1, 0.08, 0.06})) --range from -1 to +1
	local valB = (SimplexNoise.Noise2DCompound(rx/3*Config.terrainScale-24598, ry/3*Config.terrainScale+10145, {1.25, 1.5, 2, 3, 4, 15, 25, 35, 60}, {0.8, 0.95, 0.75, 0.55, 0.4, 0.25, 0.15, 0.08, 0.06})) --range from -1 to +1
	...
	if (math.abs(valA) < 0.2 or math.abs(valB) < 0.2) and math.abs(val2) > 0.25 then
		return ((math.abs(valA) < 0.1 or math.abs(valB) < 0.1) and math.abs(val2) > 0.4 and val3 <= 0) and 2 or 1
	end
	return 0
end
